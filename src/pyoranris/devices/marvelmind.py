...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="/dev/ttyACM0", baud=9600, maxvaluescount=1, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    def print_position(self):
        if (self.position()[6]):
            ...
            tt = time.time()
            print ("Hedge {:d}: X: {:.3f} m, Y: {:.3f} m, Z: {:.3f} m, Angle: {:d} at time T: {:%Y-%m-%d %H:%M:%S}-{:03d} Raw: {:.3f}".format(self.position()[0], self.position()[1], self.position()[2], self.position()[3], self.position()[4], dt, tmsec,tt))
            #print(f'Raw Time: {tt}')
            ...
            print(f'Raw Time: {time.time()}')
        
    def print_position_1(self):
        # if (self.position()[6]):
        #     tsec= int(math.trunc(self.position()[5]/1000.0))
        #     tmsec= int(self.position()[5] % 1000)
        #     dt = datetime.datetime.utcfromtimestamp(tsec)
        ...
        #     print ("Hedge {:d}: X: {:.3f}, Y: {:.3f}, Z: {:.3f}, Angle: {:d} at time T: {:.3f}".format(self.position()[0], self.position()[1], self.position()[2], self.position()[3], self.position()[4], self.position()[5]/1000.0))
        #   #  print(f'Raw Time: {time.time()}')  
        return self.position(),time.time()
